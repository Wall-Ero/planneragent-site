...
const boundedConversation = /^(?:(?:hello|hi|hey)(?:\s+planneragent)?[!.?]*|(?:are you ready|who are you|are you there|can you hear me)[?.!]*|(?:ok(?:ay)?|understood|got it|thanks|thank you)[!.?]*)$/i;
const descriptiveOperationalContext = /\b(?:i(?:'m| am)|we(?:'re| are)|our)\b[\s\S]{0,240}\b(?:supplier|inventory|order|production|delivery|schedule|warehouse|logistics|finance|planning|operations?)\b[\s\S]{0,240}\b(?:miss(?:ing)?|late|delay(?:ed|s)?|slip(?:ping)?|short(?:age)?|blocked|unstable|pressure|risk|problem|issue|facing)\b|\b(?:supplier|inventory|order|production|delivery|schedule|warehouse|logistics|finance|planning|operations?)\b[\s\S]{0,240}\b(?:miss(?:ing)?|late|delay(?:ed|s)?|slip(?:ping)?|short(?:age)?|blocked|unstable|pressure|risk|problem|issue)\b/i;
...
export function admitAnonymousVisionConversationV1(value: unknown): Readonly<{ request: AnonymousVisionConversationRequestV1; admission: AnonymousConversationAdmissionV1 }> | undefined {
  ...
  const admission = protectedRequest.test(message) ? "PROTECTED_DISCLOSURE" : data.test(message) ? "DATA_INTRODUCTION" : execution.test(message) ? "EXECUTION_REQUEST" : boundedConversation.test(message) ? "BOUNDED_CONVERSATION" : descriptiveOperationalContext.test(message) ? "DESCRIPTIVE_OPERATIONAL_CONTEXT" : product.test(message) ? "PRODUCT_CONVERSATION" : undefined;
