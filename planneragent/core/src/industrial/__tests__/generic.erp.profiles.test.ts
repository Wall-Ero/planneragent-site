...
describe("WU12C generic read-only ERP profiles", () => {
  ...
  it("admits optional authoritative planned output availability without deriving a fallback",async()=>{const legacy=await run("PRODUCTION_ORDERS")as any;expect(legacy.productionOrders[0]).not.toHaveProperty("planned_output_available_at");expect(legacy.cognitionRows[0]).not.toHaveProperty("planned_output_available_at");expect(legacy.governedFacts[0].semantic).not.toHaveProperty("planned_output_available_at");const at="2026-08-01T08:00:00.000Z",row={...rows.PRODUCTION_ORDERS,planned_output_available_at:at},output=await run("PRODUCTION_ORDERS",async()=>response("PRODUCTION_ORDERS",[row]))as any;expect(output.productionOrders[0].planned_output_available_at).toBe(at);expect(output.cognitionRows[0].planned_output_available_at).toBe(at);expect(output.governedFacts[0].semantic).toMatchObject({production_order_id:"P1",article:"FG1",quantity:{value:5,source_unit:"EA",canonical_unit:"EACH"},planned_output_available_at:at});expect(output.governedFacts[0].semantic.scheduled_at).toBe(rows.PRODUCTION_ORDERS.scheduled_at);});
  it("rejects invalid planned output availability and keeps it truth-bearing",async()=>{await denied(run("PRODUCTION_ORDERS",async()=>response("PRODUCTION_ORDERS",[{...rows.PRODUCTION_ORDERS,planned_output_available_at:"2026-08-01"}])) ,"ERP_DATE_AMBIGUOUS");const a=await run("PRODUCTION_ORDERS",async()=>response("PRODUCTION_ORDERS",[{...rows.PRODUCTION_ORDERS,planned_output_available_at:"2026-08-01T08:00:00.000Z"}]))as any,b=await run("PRODUCTION_ORDERS",async()=>response("PRODUCTION_ORDERS",[{...rows.PRODUCTION_ORDERS,planned_output_available_at:"2026-08-01T09:00:00.000Z"}]))as any,replay=await run("PRODUCTION_ORDERS",async()=>response("PRODUCTION_ORDERS",[{...rows.PRODUCTION_ORDERS,planned_output_available_at:"2026-08-01T08:00:00.000Z"}]))as any;expect(a.governedFacts[0].fact_id).toBe(b.governedFacts[0].fact_id);expect(a.governedFacts[0].fact_digest).not.toBe(b.governedFacts[0].fact_digest);expect(replay.governedFacts[0]).toEqual(a.governedFacts[0]);});
