...
describe("WU11A-1 — Governed Industrial Dataset Admission", () => {
  ...
  it("admits optional file planned output availability, omits blanks, and rejects invalid values",()=>{const headers=Object.freeze(["order","article","quantity","planned_output_available_at"]),at="2026-08-20T10:00:00.000Z",valid=admit(data({headers,rows:Object.freeze([Object.freeze(["P1","FG1","5",at]),Object.freeze(["P2","FG2","3",""])])}),"PRODUCTION_ORDERS");expect(valid.admitted).toBe(true);if(valid.admitted){const rows=(adaptAdmittedDatasetToCognitionInput(valid.dataset)as any).movord;expect(rows[0].planned_output_available_at).toBe(at);expect(rows[1]).not.toHaveProperty("planned_output_available_at");}expect(admit(data({headers,rows:Object.freeze([Object.freeze(["P1","FG1","5","2026-08-20"])])}),"PRODUCTION_ORDERS")).toEqual({admitted:false,failure:"DATASET_STRUCTURE_INCOMPATIBLE"});expect(admit(data({headers:Object.freeze([...headers,"simulator_eta"]),rows:Object.freeze([Object.freeze(["P1","FG1","5",at,at])])}),"PRODUCTION_ORDERS")).toEqual({admitted:false,failure:"DATASET_HEADERS_INVALID"});});
