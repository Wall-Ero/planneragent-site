...
export async function verifyOperationsBaselineCommitmentFeasibilityV1(value:OperationsBaselineCommitmentFeasibilityV1):Promise<void>{
  const semantic={version:value.version,baseline_semantics:value.baseline_semantics,protected_objective_ref:value.protected_objective_ref,protected_objective_digest:value.protected_objective_digest,order_fact_ref:value.order_fact_ref,order_version_ref:value.order_version_ref,item_ref:value.item_ref,committed_quantity:value.committed_quantity,commitment_boundary:value.commitment_boundary,result:value.result,allocated_quantity:value.allocated_quantity,shortage_quantity:value.shortage_quantity,evidence_boundary:value.evidence_boundary,evidence_refs:value.evidence_refs,qualification_refs:value.qualification_refs,provenance_refs:value.provenance_refs,causal_lineage_refs:value.causal_lineage_refs,request_id:value.request_id,company_id:value.company_id,scope_id:value.scope_id,scope_digest:value.scope_digest,evidence_selection_ref:value.evidence_selection_ref,evidence_as_of:value.evidence_as_of,evaluated_at:value.evaluated_at};
  const expected=await sha(semantic);
  if(value.feasibility_digest!==expected||value.feasibility_id!==`operations-baseline-commitment-feasibility:sha256:${expected}`||value.digest_algorithm!=="SHA-256"||value.observational_only!==true||value.grants_execution!==false||value.grants_authority!==false||value.grants_remediation!==false||value.company_global_claim!==false)throw new Error("OPS_BASELINE_FEASIBILITY_INTEGRITY_INVALID");
}
