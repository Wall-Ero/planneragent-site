...
import { deliverEmailViaResendBridgeV1, type ResendBridgeSenderV1 } from "./email.resend.delivery.bridge.v1";
...
export async function emailResendRelayRouteV1(request: Request, env: Env, send?: ResendBridgeSenderV1): Promise<Response> {
  ...
  const result = await deliverEmailViaResendBridgeV1(env, { to: value.to, subject: value.subject, body: value.body }, send);
  if (result.ok) return json({ version: 1, status: result.status }, 202);
  const reason = "reason" in result ? result.reason : "DELIVERY_FAILED";
  return json({ version: 1, error: reason }, reason === "REQUEST_INVALID" ? 400 : reason === "DELIVERY_NOT_CONFIGURED" ? 503 : 502);
}
