...
export async function applyGovernedPlanRealityMaterialityPolicyV1(input:Readonly<{
  ...
  const objective=input.protected_objective,assessment=input.objective_assessment;
  ...
  let disposition:GovernedPlanRealityMaterialityDispositionV1="NOT_ASSERTABLE",reason_codes:GovernedPlanRealityMaterialityReasonV1[]=["ELEMENT_COMPARISON_NOT_ASSERTABLE"],supports_element_alignment=false,supports_element_misalignment=false;
  if(comparison.comparison_status==="ASSERTABLE"&&comparison.disposition==="EXACT_TARGET_SATISFIED"&&assessment?.assessment_status==="ASSESSED"&&assessment.objective_state==="BREACHED"){
    ...
    if(assessment?.assessment_status==="ASSESSED"&&assessment.objective_state==="BREACHED"){
      ...
      disposition="DEVIATION_RECOVERABILITY_UNRESOLVED";reason_codes=["DRIFTING_RECOVERABILITY_POLICY_REQUIRED"];
    }else{disposition="NOT_ASSERTABLE";reason_codes=["OBJECTIVE_STATE_UNRESOLVED"]}
  }
  const evidence_refs=refs([...comparison.evidence_refs,...(assessment?.evidence_refs??[])]),qualification_refs=refs([...comparison.qualification_refs,...(assessment?.qualification_refs??[])]),provenance_refs=refs([...comparison.provenance_refs,...(assessment?.provenance_refs??[])]),causal_lineage_refs=refs([...comparison.causal_lineage_refs,...(assessment?.causal_lineage_refs??[]),comparison.comparison_id,...(objective?[objective.objective_binding_id]:[]),...(assessment?[assessment.assessment_id]:[])]);
  const semantic={version:1 as const,policy_ref:"policy:governed-plan-reality-element-materiality-v1"as const,comparison_ref:comparison.comparison_id,comparison_digest:comparison.comparison_digest,applicable_plan_context_ref:comparison.applicable_plan_context_ref,plan_element_ref:comparison.plan_element_ref,...(objective?{protected_objective_ref:objective.objective_binding_id,protected_objective_digest:objective.objective_binding_digest}:{}),...(assessment?{objective_assessment_ref:assessment.assessment_id,objective_assessment_digest:assessment.assessment_digest}:{}),request_id:comparison.request_id,company_id:comparison.company_id,scope_id:comparison.scope_id,scope_digest:comparison.scope_digest,evidence_as_of:comparison.evidence_as_of,disposition,reason_codes:Object.freeze(reason_codes),supports_element_alignment,supports_element_drifting:false as const,supports_element_misalignment,scope_aggregation_performed:false as const,source_count_independent:true as const,decision_pressure_independent:true as const,intrinsic_reality_state_independent:true as const,evidence_refs,qualification_refs,provenance_refs,causal_lineage_refs};
