...
import { CognitiveTransportError,type SealedCognitiveExposureV1 } from '../../governance/knowledge-exposure/transport';
import {
...
export async function executeWithLlmProviders(
	...
	for (let i = 0; i < providers.length; i++) {
		...
		try {
			...
			if(securityEvidenceRepository&&runtimeContext){const c=runtimeContext.security_evidence_context,ce=evidencedResolver?.latest.get(runtimeContext.binding_reference.binding_id),boundary=err instanceof ProviderBoundaryFailureV1,transportEvidenceId=err instanceof CognitiveTransportError?err.transport_evidence_id:undefined;const attempt=Object.freeze({version:1,attempt_id:attemptId,request_id:requestId,sequence:i,...(technicalPredecessor?{predecessor_attempt_id:technicalPredecessor.attempt_id}:{}),runtime_binding_id:runtimeContext.causal_reference.provider_runtime_binding_id,runtime_binding_digest:runtimeContext.causal_reference.provider_runtime_binding_digest,admission_id:c?.admission_id,admission_digest:c?.admission_digest,oks_consumption_id:c?.oks_consumption_id,credential_access_evidence_id:ce?.evidence_id,provider:candidate.id,provider_account_id:runtimeContext.binding_reference.provider_account_id,provider_deployment_id:runtimeContext.binding_reference.provider_deployment_id,adapter_identity:runtimeContext.binding_reference.adapter_identity,model:runtimeContext.binding_reference.selected_model,outcome:ce?.outcome==='RESOLUTION_FAILED'?'CREDENTIAL_RESOLUTION_FAILED':boundary?'CONFIGURATION_FAILED':'TRANSPORT_FAILED',failure_class:boundary?'CONFIGURATION':'TECHNICAL',failure_code:boundary?err.code:ce?.failure_code??(err instanceof CognitiveTransportError?err.code:'PROVIDER_TECHNICAL_FAILURE'),...(transportEvidenceId?{transport_evidence_id:transportEvidenceId}:{}),started_at:startedAt,completed_at:now(),correlation_id:c?.correlation_id??requestId,causal_references:c?.causal_references??[]}) as ProviderRuntimeAttemptEvidenceV1;await securityEvidenceRepository.persistAttempt(attempt);if(technicalPredecessor)await securityEvidenceRepository.persistFallback({lineage_id:`provider-runtime-fallback:${technicalPredecessor.attempt_id}:${attempt.attempt_id}`,predecessor_attempt_id:technicalPredecessor.attempt_id,successor_attempt_id:attempt.attempt_id,reason:'TECHNICAL_FAILURE',linked_at:attempt.started_at,correlation_id:attempt.correlation_id});if(!boundary)technicalPredecessor=attempt;}
