  ...
  async fetch(req: Request, env: Env): Promise<Response> {
    try {
      ...
      if (url.pathname.startsWith("/onboarding")) {
        const subPath = url.pathname.replace("/onboarding", "") || "/";
        const onboardingUrl = new URL(req.url);
        onboardingUrl.pathname = subPath;

        const onboardingReq = new Request(onboardingUrl.toString(), req);
        return onboardingRoutes.fetch(onboardingReq, env);
      }
      ...
      const snapshotUnsigned = {
        ...
        // 🔥 QUI PASSA IL BEHAVIOR
        behavior_override: parsed.behavior_override,
