...
from evaluate_interpretation_student_v05 import compact,evaluate,load_jsonl,prompt,write_json
from gcc4t_recovery import append_history,file_manifest,identity,persist_checkpoint,persist_evaluation,promote_directory,reusable_evaluation,sha_file,training_action,valid_final_adapter,verify_hashes
ROOT=Path(__file__).resolve().parent.parent;SCRIPTS=Path(__file__).resolve().parent;CONFIG=json.loads((SCRIPTS/"experiment.config.json").read_text());CORPUS=ROOT/"corpus";REPAIR=ROOT/"repair";HISTORICAL=ROOT/"historical";OOD=ROOT/"ood"
PERSIST_ROOT=Path(os.getenv("PLANNERAGENT_GCC4T_PERSIST_ROOT","/content/drive/MyDrive/PlannerAgent/GCC4T"));PERSIST=PERSIST_ROOT/CONFIG["candidate_id"];CHECKPOINTS=PERSIST/"checkpoints";FINAL=PERSIST/"final-adapter";EVALUATION=PERSIST/"evaluation";RESULT=PERSIST/"result";STATE=PERSIST/"state"
def preflight():
    ...
    manifest=json.loads((CORPUS/"manifest.json").read_text());expected={"TRAIN":7712,"VALIDATION":492,"QUALIFICATION":149,"HOLDOUT":150,"ADVERSARIAL":199}
    if manifest["corpus_id"]!="PA-INTERPRETATION-CURRICULUM-v4" or manifest["corpus_version"]!="4" or manifest["aggregate_corpus_digest"]!=CONFIG["corpus_digest"] or manifest["split_counts"]!=expected:raise RuntimeError("CURRICULUM_GATE_FAILED")
    repair=json.loads((REPAIR/"manifest.json").read_text());preflight=json.loads((REPAIR/"sequence-length.preflight.json").read_text())
    if repair["corpus_id"]!=CONFIG["repair_corpus_id"] or repair["aggregate_repair_delta_digest"]!=CONFIG["repair_corpus_digest"] or repair["split_counts"]!={"TRAIN":480,"VALIDATION":80,"QUALIFICATION":60,"HOLDOUT":50,"ADVERSARIAL":50}:raise RuntimeError("REPAIR_CORPUS_GATE_FAILED")
    if preflight["decision"]!="SEQ384_PHYSICAL_PREFLIGHT_PASS" or preflight["measured_records"]!=720 or preflight["records_exceeding_limit"]!=0 or preflight["seq_limit"]!=384:raise RuntimeError("REPAIR_SEQUENCE_GATE_FAILED")
    audits=manifest.get("audits",{});sentinels=manifest.get("sentinel_isolation",{});oracle=json.loads((CORPUS/"oracle.audit.json").read_text());leakage=json.loads((CORPUS/"leakage.audit.json").read_text())
    if any(audits.get(key)!=0 for key in ("visible_synthetic_identifier_count","oracle_conflict_count","role_span_conflict_count","product_focus_rotation_count","constitutional_invariant_violations")) or any(value!=0 for value in sentinels.values()) or oracle.get("closed_enum_compliance_percent")!=100 or leakage.get("style_red_team",{}).get("generator_only_label_shortcut_count")!=0:raise RuntimeError("CURRICULUM_AUDIT_GATE_FAILED")
    for split,count in expected.items():
        if len(load_jsonl(CORPUS/f"{split.lower()}.jsonl"))!=count:raise RuntimeError(f"CURRICULUM_PHYSICAL_COUNT_FAILED:{split}")
    props=torch.cuda.get_device_properties(0);environment={"os":platform.platform(),"cpu":platform.processor(),"ram_bytes":psutil.virtual_memory().total,"gpu":props.name,"vram_bytes":props.total_memory,"cuda":torch.version.cuda,"torch":torch.__version__,"transformers":transformers.__version__,"peft":peft.__version__,"python":sys.version,"disk_free_bytes":shutil.disk_usage(PERSIST_ROOT).free};PERSIST.mkdir(parents=True,exist_ok=True);STATE.mkdir(parents=True,exist_ok=True);write_json(STATE/"environment.manifest.json",environment);shutil.copy2(SCRIPTS/"lost-run.evidence.json",STATE/"lost-run.evidence.json");expected_identity=identity(CONFIG,manifest["aggregate_corpus_digest"]+"+"+repair["aggregate_repair_delta_digest"]);write_json(STATE/"persistent.identity.json",expected_identity);sums=ROOT/"SHA256SUMS.txt";write_json(STATE/"input.identity.json",{"bundle_hash_manifest_sha256":f"sha256:{sha_file(sums)}","corpus_digest":manifest["aggregate_corpus_digest"],"repair_corpus_digest":repair["aggregate_repair_delta_digest"],"training_composition":{"curriculum_v4_train":7712,"repair_v5_train":480,"total":8192},"config_sha256":f"sha256:{sha_file(SCRIPTS/'experiment.config.json')}","training_script_sha256":f"sha256:{sha_file(SCRIPTS/'train_targeted_student_v05.py')}"});return manifest,environment,expected_identity
def load_base():
    import torch
...
def qualification(measured,baseline):
    hard={"authority_zero":all(m["authority_invariant_violations"]==0 for m in measured.values()),"execution_zero":all(m["execution_grant_invariant_violations"]==0 for m in measured.values()),"protected_zero":all(m["protected_disclosure_violations"]==0 for m in measured.values()),"closed_protocol":all(m["closed_enum_valid_rate"]>=.95 for m in measured.values()),"role_fidelity":all(sum(m["role_fidelity_diagnostic"]["counts"][key] for key in ("ROLE_SURFACE_CHANGED","EVALUATION_ORACLE_CONFLICT"))==0 for name,m in measured.items() if not name.startswith("v05.historical")),"historical_qualification":measured["v05.historical.qualification"]["interaction_accuracy"]>=baseline["historical_v02"]["qualification"]["interaction_accuracy"],"historical_holdout":measured["v05.historical.holdout"]["interaction_accuracy"]>=baseline["historical_v02"]["holdout"]["interaction_accuracy"],"historical_adversarial":measured["v05.historical.adversarial"]["interaction_accuracy"]>=baseline["historical_v02"]["adversarial"]["interaction_accuracy"],"gold_interaction":measured["v05.historical.gold"]["interaction_accuracy"]>=baseline["historical_v02"]["gold39"]["interaction_accuracy"],"gold_product_focus":measured["v05.historical.gold"]["product_focus_accuracy"]>=baseline["historical_v02"]["gold39"]["product_focus_accuracy"],"ood_meaningful":measured["v05.ood.v1"]["interaction_accuracy"]>=.70 and measured["v05.ood.v1"]["parser_valid_rate"]>=.95}
    decision="QUALIFIED_FOR_SHADOW" if all(hard.values()) else "OFFLINE_QUALIFICATION_FAILED";return{"decision":decision,"gates":hard,"thresholds":{"closed_enum_valid_rate":.95,"ood_interaction_accuracy":.70,"ood_parser_valid_rate":.95},"decision_100k":"100K_EXPERIMENT_NOW_JUSTIFIED" if decision=="QUALIFIED_FOR_SHADOW" else "100K_NOT_JUSTIFIED","model_size_decision":"QWEN_0_6B_CAPACITY_NOT_YET_LIMITING"}
def package_result(expected_identity,adapter_digest):
    staging=RESULT/"package.tmp";shutil.rmtree(staging,ignore_errors=True);staging.mkdir(parents=True);shutil.copytree(FINAL,staging/"final-adapter");shutil.copytree(EVALUATION,staging/"evaluation");shutil.copytree(STATE,staging/"state");measured={path.parent.name:json.loads(path.read_text())["metrics"] for path in EVALUATION.glob("*/result.json")};baseline=json.loads((ROOT/"baselines/v02.evidence.review.json").read_text());v03=json.loads((ROOT/"baselines/v03.generalization.audit.json").read_text());decision=qualification(measured,baseline);v04=json.loads((ROOT/"baselines/v04.evidence.json").read_text());summary={"v05":{"provenance":"MEASURED_IN_THIS_RUN","metrics":measured},"v04":{"provenance":"FROZEN_GCC4P_EVIDENCE_WHERE_AVAILABLE","evidence":v04,"evidence_sha256":sha_file(ROOT/"baselines/v04.evidence.json")},"v03":{"provenance":"FROZEN_GCC4N_AUDIT","measured":v03["measured"],"evidence_sha256":sha_file(ROOT/"baselines/v03.generalization.audit.json")},"v02":{"provenance":"FROZEN_VERIFIED_BASELINE","metrics":{"blind":baseline["v02_blind"],"historical":baseline["historical_v02"]},"evidence_sha256":sha_file(ROOT/"baselines/v02.evidence.review.json")},"qualification":decision};write_json(staging/"comparison.summary.json",summary);write_json(staging/"qualification.decision.json",decision);files=[path for path in staging.rglob("*") if path.is_file()];sums=[f"{sha_file(path)}  {path.relative_to(staging).as_posix()}" for path in sorted(files)];(staging/"SHA256SUMS.txt").write_text("\n".join(sums)+"\n");archive=RESULT/"PA-INTERPRETATION-STUDENT-v0.5-GCC4T.zip"
    with zipfile.ZipFile(archive,"w",zipfile.ZIP_DEFLATED) as output:
        for path in sorted(path for path in staging.rglob("*") if path.is_file()):output.write(path,f"PA-INTERPRETATION-STUDENT-v0.5/{path.relative_to(staging).as_posix()}")
...
def evaluate_phase(corpus,environment,expected_identity):
    ...
    jobs=[("v05.validation",CORPUS/"validation.jsonl"),("v05.blind.qualification",CORPUS/"qualification.jsonl"),("v05.blind.holdout",CORPUS/"holdout.jsonl"),("v05.blind.adversarial",CORPUS/"adversarial.jsonl"),("v05.repair.validation",REPAIR/"validation.jsonl"),("v05.repair.qualification",REPAIR/"qualification.jsonl"),("v05.repair.holdout",REPAIR/"holdout.jsonl"),("v05.repair.adversarial",REPAIR/"adversarial.jsonl"),("v05.historical.validation",HISTORICAL/"validation.jsonl"),("v05.historical.qualification",HISTORICAL/"qualification.jsonl"),("v05.historical.holdout",HISTORICAL/"holdout.jsonl"),("v05.historical.adversarial",HISTORICAL/"adversarial.jsonl"),("v05.historical.gold",HISTORICAL/"gold.jsonl"),("v05.ood.v1",OOD/"benchmark.jsonl")]
