...
from gcc4k_recovery import append_history,file_manifest,identity,persist_checkpoint,persist_evaluation,promote_directory,reusable_evaluation,sha_file,training_action,valid_final_adapter,verify_hashes
...
PERSIST_ROOT=Path(os.getenv("PLANNERAGENT_GCC4K_PERSIST_ROOT","/content/drive/MyDrive/PlannerAgent/GCC4K"));PERSIST=PERSIST_ROOT/CONFIG["candidate_id"];CHECKPOINTS=PERSIST/"checkpoints";FINAL=PERSIST/"final-adapter";EVALUATION=PERSIST/"evaluation";RESULT=PERSIST/"result";STATE=PERSIST/"state"
def preflight():
...
def persist_final(model,tokenizer,expected_identity,environment,training_metrics,training_config):
    ...
    RESULT.mkdir(parents=True,exist_ok=True);adapter_zip=RESULT/"PA-INTERPRETATION-STUDENT-v0.2-ADAPTER-GCC4K.zip"
    with zipfile.ZipFile(adapter_zip,"w",zipfile.ZIP_DEFLATED) as archive:
        for path in sorted(path for path in FINAL.rglob("*") if path.is_file()):archive.write(path,f"PA-INTERPRETATION-STUDENT-v0.2-ADAPTER/{path.relative_to(FINAL).as_posix()}")
...
def openai_job(name,dataset_path,expected_identity,adapter_digest,key,reference):
    evaluation_config_digest=f"sha256:{hashlib.sha256(compact({'provider':'openai','model':reference,'training_use':False}).encode()).hexdigest()}";dataset_digest=f"sha256:{sha_file(dataset_path)}";target=EVALUATION/name;cached=reusable_evaluation(target,expected_identity,adapter_digest,dataset_digest,evaluation_config_digest)
    if cached is not None:return cached["metrics"]
    result=benchmark_openai(load_jsonl(dataset_path),key,reference);persist_evaluation(target,expected_identity,adapter_digest,dataset_digest,evaluation_config_digest,{"dataset_identity":name,"candidate_id":CONFIG["candidate_id"],"evaluation_config":{"provider":"openai","model":reference,"training_use":False},**result});return result["metrics"]
def package_result(expected_identity,adapter_digest):
    staging=RESULT/"package.tmp";shutil.rmtree(staging,ignore_errors=True);staging.mkdir(parents=True);shutil.copytree(FINAL,staging/"final-adapter");shutil.copytree(EVALUATION,staging/"evaluation");shutil.copytree(STATE,staging/"state");summary={path.parent.name:json.loads(path.read_text())["metrics"] for path in EVALUATION.glob("*/result.json")};write_json(staging/"comparison.summary.json",summary);files=[path for path in staging.rglob("*") if path.is_file()];sums=[f"{sha_file(path)}  {path.relative_to(staging).as_posix()}" for path in sorted(files)];(staging/"SHA256SUMS.txt").write_text("\n".join(sums)+"\n");archive=RESULT/"PA-INTERPRETATION-STUDENT-v0.2-GCC4K.zip"
    with zipfile.ZipFile(archive,"w",zipfile.ZIP_DEFLATED) as output:
        for path in sorted(path for path in staging.rglob("*") if path.is_file()):output.write(path,f"PA-INTERPRETATION-STUDENT-v0.2/{path.relative_to(staging).as_posix()}")
...
def evaluate_phase(corpus,environment,expected_identity):
    ...
    jobs=[("v02.validation",CORPUS/"validation.jsonl"),("v02.blind.qualification",CORPUS/"qualification.jsonl"),("v02.blind.holdout",CORPUS/"holdout.jsonl"),("v02.blind.adversarial",CORPUS/"adversarial.jsonl"),("v02.historical.validation",HISTORICAL/"validation.jsonl"),("v02.historical.qualification",HISTORICAL/"qualification.jsonl"),("v02.historical.holdout",HISTORICAL/"holdout.jsonl"),("v02.historical.adversarial",HISTORICAL/"adversarial.jsonl"),("v02.historical.gold",HISTORICAL/"gold.jsonl")]
    for name,path in jobs:evaluation_job(model,tokenizer,name,path,expected_identity,manifest["adapter_digest"])
    del model,base;gc.collect();torch.cuda.empty_cache();v01=ROOT/"v01"/"adapter"
    if v01.exists():
        base,tokenizer,_=load_base();comparison=PeftModel.from_pretrained(base,v01).cuda();v01_digest=f"sha256:{sha_file(v01/'adapter_model.safetensors')}"
        for name in ("qualification","holdout","adversarial"):evaluation_job(comparison,tokenizer,f"v01.blind.{name}",CORPUS/f"{name}.jsonl",expected_identity,v01_digest)
        del comparison,base;gc.collect();torch.cuda.empty_cache()
    key=os.getenv("OPENAI_API_KEY");reference=os.getenv("OPENAI_CONVERSATIONAL_MODEL")
    if key and reference:
        for name in ("qualification","holdout","adversarial"):openai_job(f"openai.blind.{name}",CORPUS/f"{name}.jsonl",expected_identity,manifest["adapter_digest"],key,reference)
    else:write_json(STATE/"openai.status.json",{"status":"UNAVAILABLE","reason":"missing explicit OPENAI_API_KEY and/or OPENAI_CONVERSATIONAL_MODEL"})
