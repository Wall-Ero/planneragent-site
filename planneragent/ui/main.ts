...
async function bootstrap(){const root=document.querySelector<HTMLElement>("#app");if(!root)return;const session=getSession(),role=resolveRoleFromSession(session),presentation=createCockpitPresentation(await currentCockpit());
 root.innerHTML=`<main class="cockpit-shell" data-status="${presentation.available?"available":"waiting"}">
  ...
  <section class="activity" aria-label="PlannerAgent activity: ${presentation.activity}">${frame(frames.activity,"activity")}<div class="activity-line"><i></i></div></section>
  ...
  <section class="cognition" aria-label="Operational cockpit signals">
   <article class="signal-panel">${frame(frames.plan,"plan")}<h2>PLAN</h2><div class="vertical-signals">${signal("VALID",presentation.plan)}${signal("MISSING",presentation.plan)}${signal("BROKEN",presentation.plan)}</div></article>
   ...
   <article class="signal-panel">${frame(frames.reality,"reality")}<h2>REALITY</h2><div class="vertical-signals">${signal("STABLE",presentation.reality)}${signal("SHIFTING",presentation.reality)}${signal("UNSTABLE",presentation.reality)}</div></article>
  </section>
  ...
  <form class="composer">${frame(frames.input,"chat-input")}<button type="button" class="composer-button add" aria-label="Attach data">+</button><label class="sr-only" for="message">Message PlannerAgent</label><input id="message" name="message" autocomplete="off" placeholder="Type here..."><button type="button" class="composer-button microphone" aria-label="Microphone">${frame(frames.microphone,"microphone")}</button></form>
  <section class="governance" aria-labelledby="governance-title"><div class="governance-title-row"><span class="governance-rule">${frame(frames.governance,"governance-rule")}</span><h2 id="governance-title">AI OPERATIONAL GOVERNANCE</h2><span class="governance-rule">${frame(frames.governance,"governance-rule")}</span></div><div class="governance-mode-row"><p>Mode: VISION. Observation only. No execution.</p><button class="help" type="button" aria-label="Help">?</button></div><div class="governance-graduate-row">GRADUATE: OFF</div></section>
 </main>`;
